...
# 基础库
import os, sys, platform, shutil, datetime, time, locale
import threading, func_timeout, io, mmap, re, subprocess, elevate
...
import pythoncom, ctypes, win32api, win32con, win32file, win32pipe
# 硬件检测的库
import psutil
...
def check_fs(path:str) -> None:
    ...
    fs_type = fs_dict.get(os.path.splitdrive(path)[0] , "")
...
def exiftool_enhance_process(src_file_path: str , Initial_processed_image_fd: io.BytesIO , output_file_path: str) -> None:
    ...
    try:
        ...
        err = err.decode(encoding=cmd_encoding , errors="replace")
...
def pic_save(output_path: Optional[str] = "" , preserve_exif: Optional[bool] = True , exif_rotate: Optional[bool] = True , output_to_memory_io: Optional[bool] = False) -> Union[io.BytesIO , None]:
    ...
    if preserve_exif:
        ...
        if output_to_memory_io:
            ret_io = io.BytesIO()
            # 保存，参数帮助：https://pillow.readthedocs.io/en/stable/handbook/image-file-formats.html#jpeg-saving
            im.save(fp=ret_io , format=target_format , quality=jpg_quality , subsampling=jpg_subsample_option )
            # 返回装载数据的io供exiftool使用
            return ret_io
        else:
            with io.BytesIO() as tmp_io:
...
if not bool(ctypes.WinDLL("shell32.dll").IsUserAnAdmin()):
    handle_critical_error("错误，程序未取得管理员权限" , log_handle_present=False)
...
with log_handle:
    ...
    try:
        ...
        for partition in psutil.disk_partitions(all=True):
            fs_dict[getattr(partition,"mountpoint","").rstrip("\\")] = getattr(partition,"fstype","")
                                                        ...
                                                        if pdf_inside_pic_direct_output:
                                                            ...
                                                            with Image.open(file_data) as im:
                                                                if (im.format not in runtime_excluded_format_set) and has_only_one_frame(im):
                                                                    ...
                                                                    file_data.close()
                                                                    # 将im中的图片保存
                                                                    pic_save(output_path=f"{inside_img_output_path}\\{str(pic_num).zfill(3)}.{target_ext}" , preserve_exif=perserve_common_pic_exif)
                                                                else:
