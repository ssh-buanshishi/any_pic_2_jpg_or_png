...
if (ret := os.system(f"fsutil file createNew \"{output_file}\" 32768 >nul")):
    print(f"无法给【{output_file}】预分配空间")
    print("\n\n按任意键结束")
    os.system("@pause>nul")
    sys.exit(0)

with open(output_file , mode="tr+" , encoding="utf-8-sig" , errors="replace" , newline="\r\n") as f:
